...
tp_obj tp_bitwise_and(TP,tp_obj _a,tp_obj _b);
tp_obj tp_bitwise_or(TP,tp_obj _a,tp_obj _b);
tp_obj tp_bitwise_xor(TP,tp_obj _a,tp_obj _b);
...
tp_vm *_tp_init(void) {
    ...
    tp_set(tp,tp->root,tp_None,tp->_frames);
...
void tp_frame(TP,tp_obj globals,tp_obj code,tp_obj *ret_dest) {
    tp_obj frame_obj = tp_None;
    frame_obj.type = TP_FRAME;
    tp_frame_ *f = tp_malloc(tp, sizeof(tp_frame_));
    memset(f, 0, sizeof(tp_frame_));   
    frame_obj.frame.val = f;
		
    f->globals = globals;
    f->code = code;
    f->cur = (tp_code*)f->code.string.val;
    f->jmp = 0;
    if( tp_frame_count(tp) == 0 ) {
        f->regs = tp->regs;
    }
    else {
        tp_frame_ *frame = tp_frame_current(tp);
        f->regs = frame->regs + frame->cregs;
    }
    ...
    if (f->regs+(256+TP_REGS_EXTRA) >= tp->regs+TP_REGS || tp_frame_count(tp) >= TP_FRAMES-1) {
        tp_raise(,tp_string("(tp_frame) RuntimeError: stack overflow"));
    }

    tp_set(tp, tp->_frames, tp_None, frame_obj);
}
...
void tp_print_stack(TP) {
    int i;
    printf("\n");
    for (i=0; i < tp_frame_count(tp) ; i++) {
        tp_frame_ *frame = tp_frame_get(tp, i);
        if (!frame->lineno) { continue; }
        printf("File \""); tp_echo(tp,frame->fname); printf("\", ");
        printf("line %d, in ",frame->lineno);
        tp_echo(tp,frame->name); printf("\n ");
        tp_echo(tp,frame->line); printf("\n");
    }
...
void tp_handle(TP) {
    int i;
    tp_frame_ *f;
    for (i=tp_frame_count(tp); i>0; i--) {
        f = tp_frame_get(tp, i - 1);
        if (f->jmp) { break; }
    }
    if (i > 0) {
        /* TODO: kill old frames */
        /* tp->cur_frame = i; */
        _tp_list_realloc(tp, tp->_frames.list.val, i);
        tp->_frames.list.val->len = i;
        f->cur = f->jmp;
        f->jmp = 0;
        return;
    }
...
tp_frame_* tp_frame_current(TP) {
    return tp_frame_get(tp, tp->_frames.list.val->len - 1);
}

int tp_frame_count(TP) {
    return tp->_frames.list.val->len;
}
...
tp_obj tp_call(TP,tp_obj self, tp_obj params) {
    ...
    if (self.type == TP_FNC) {
        ...
        tp_frame_ *f = tp_frame_current(tp);
        f->regs[0] = params;
        if ((self.fnc.ftype&2)) {					
            _tp_list_insert(tp,params.list.val,0,self.fnc.info->self);
        }
        tp_run(tp,tp_frame_count(tp));
        return dest;
    }
...
void tp_return(TP, tp_obj v) {
    tp_frame_ *f = tp_frame_current(tp);
    tp_obj *dest = f->ret_dest; /* tp->frames[tp->cur].ret_dest; */
...
/*     memset(tp->frames[tp->cur].regs,0,TP_REGS_PER_FRAME*sizeof(tp_obj));
    ...
    _tp_list_pop(tp, tp->_frames.list.val, tp_frame_count(tp) - 1, NULL);
    _tp_list_realloc(tp, tp->_frames.list.val, tp_frame_count(tp));
    /*
    _tp_list_pop(tp, tp->_params.list.val, tp->_params.list.val->len - 1, NULL);
    _tp_list_realloc(tp, tp->_params.list.val, tp->_params.list.val->len);
     */
    tp_free(tp, f);
    /*
    tp->cur_frame -= 1;
...
int tp_step(TP) {
    tp_frame_ *f = tp_frame_current(tp); /* &tp->frames[tp->cur]; */
    tp_obj *regs = f->regs;
    tp_code *cur = f->cur;
    while(1) {
    #ifdef TP_SANDBOX
...
void _tp_run(TP,int cur) { 
    ...
    while (tp_frame_count(tp) >= cur && tp_step(tp) != -1);
    tp->jmp -= 1;
}
...
tp_obj _tp_import(TP, tp_obj fname, tp_obj name, tp_obj code) {
    ...
    if (!tp->jmp) { tp_run(tp,tp_frame_count(tp)); }
